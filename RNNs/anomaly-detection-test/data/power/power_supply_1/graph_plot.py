...
COLUMNS = ["timestamp", "power_supply_1"]
file_name = "powerstats.gauge.PowerSupply1.csv"
...
with open(file_name, 'rb') as csvfile:
    field_names = ['time_stamp', 'power_supply_1']
...
#rescale time stamp
rows = rescale_time_stamp(rows, 1, 141000, 1, 60000)
...
for i in range(1,141000):
    ...
    y.append(float(row['power_supply_1']))
...
# min_cpu_val = min(y)
# max_cpu_val = max(y)
# avg_cpu = sum(y)/float(len(y))
# median = median(y)

# print 'Some stats on Power Supply (1) data from last 180 days'
# print 'Minimum value:   '+ str(min_cpu_val)
# print 'Maximum value:   '+ str(max_cpu_val)
# print 'Average value:   '+ str(avg_cpu)
# print 'Median :         '+ str(median)

# #plotting just cpu perc
plt.ylabel('Power Supply (1)')
plt.plot(y)
plt.grid(True)
plt.savefig('power_supply_1.png')
